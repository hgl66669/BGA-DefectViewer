...
public class OverlapParams
{
    ...
    // Derived: FOV count (auto-calculated from DeviceArea, FovSize, MoveDist)
    public int FovCountX => MoveDistX > 0
        ? Math.Max(1, (int)Math.Ceiling((DeviceAreaX - FovSizeX) / MoveDistX) + 1)
        : 1;
    public int FovCountY => MoveDistY > 0
        ? Math.Max(1, (int)Math.Ceiling((DeviceAreaY - FovSizeY) / MoveDistY) + 1)
        : 1;
    ...
    public int Alignment2FovX { get; set; } = 2;
    public int Alignment2FovY { get; set; } = 2;
}
